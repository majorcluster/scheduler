  ...
  (let [issue-date (time/date-time 2023 6 6 12 0 0)
    ...
    (testing "email not verified due to expired email token"
      (let [current-ms (time/date-time 2023 6 6 13 0 0)
            ...
            db-user (r.users/find-by-id (:user/id user))]
        (is (= 401 (:status resp)))
        (is (= {:type "unauthorized", :message "Unauthorized"}
               (-> resp :body (cjson/read-str :key-fn keyword))))
        (is (not (:user/email-verified db-user)))))
      ...
      (let [current-ms (time/date-time 2023 6 6 13 0 0)
            resp (with-redefs-fn
                   ...
                   #(response-for service
                                  ...
                                  :headers json-headers
                                  :body (cjson/write-str
                                         {:email "lenin@cccp.co" :password "11111111"})))]
        (is (= 401 (:status resp)))
        (is (= {:type "unauthorized", :message "Unauthorized"}
               (-> resp :body (cjson/read-str :key-fn keyword))))))
      ...
      (let [current-ms (time/date-time 2023 6 6 13 0 0)
            resp (with-redefs-fn
                   ...
                   #(response-for service
                                  ...
                                  :headers json-headers
                                  :body (cjson/write-str
                                         {:email "kolontai@cccp.co" :password "12345678"})))]
        (is (= 401 (:status resp)))
        (is (= {:type "unauthorized", :message "Unauthorized"}
               (-> resp :body (cjson/read-str :key-fn keyword))))))
  ...
  (let [issue-date (time/date-time 2023 6 6 12 0 0)
    ...
    (testing "when no password recovering or token is at db"
      (let [current-ms (time/date-time 2023 6 6 12 0 0)
        ...
        (is (= 401 (:status resp)))
        (is (= {:type "token-mismatch", :message "The token does not match the user"}
               (-> resp :body (cjson/read-str :key-fn keyword)))))
      (let [current-ms (time/date-time 2023 6 6 12 0 0)
    ...
    (testing "password is changed"
      (let [current-ms (time/date-time 2023 6 6 12 59 59)
        ...
        (is (nil? (:user/email-token db-user)))
        (is (= (:user/password db-user)
               "7e071fd9b023ed8f18458a73613a0834f6220bd5cc50357ba3493c6040a9ea8c"))))))

(deftest admin-signup-email-test
  (let [issue-date (time/date-time 2023 6 6 12 0 0)
        user {:user/id #uuid "3745a363-ca70-4a43-9f5f-ef0cbfdab7e4"
              :user/email-token nil
              :user/email-verified true
              :user/created-at (coerce/to-long issue-date)
              :user/role "user"
        ...
        token-admin (login issue-date
                           {:email "kollontai@cccp.co" :password "12345678"}
                           #uuid "2545a363-ca70-4a43-9f5f-ef0cbfdab7d3")]
    (testing "when mandatory data is not sent"
      (let [current-ms (time/date-time 2023 6 6 12 59 59)
            resp (with-redefs-fn
                   {#'random-uuid (fn [] #uuid "3745a363-ca70-4a43-9f5f-ef0cbfdab7e4")
...
(deftest admin-signup-test
  (let [issue-date (time/date-time 2023 6 6 12 0 0)
        email-token (l.tokens/gen-email-token-str
                     "lenin@cccp.co"
                     (coerce/to-long issue-date)
                     "email-verification")
        user {:fname "Vladmir"
